...
# Ensure both backend and root directories are in the python search path
...
def get_config():
    """Serve public configuration values like Google Books API Key and Firebase Config."""
    firebase_configured = bool(app_config.firebase.api_key and app_config.firebase.project_id)
    return jsonify({
        ...
        "firebase_config": {
            "apiKey": app_config.firebase.api_key or "",
            "authDomain": app_config.firebase.auth_domain or "",
            "projectId": app_config.firebase.project_id or "",
            "storageBucket": app_config.firebase.storage_bucket or "",
            "messagingSenderId": app_config.firebase.messaging_sender_id or "",
            "appId": app_config.firebase.app_id or ""
        }
    })
    ...
    if firebase_configured:
        try:
            if not firebase_admin._apps:
                if app_config.firebase.credentials_json:
                    import json
                    if os.path.exists(app_config.firebase.credentials_json):
                        cred = credentials.Certificate(app_config.firebase.credentials_json)
                    else:
                        cred_dict = json.loads(app_config.firebase.credentials_json)
...
@app.route('/api/v1/auth/firebase', methods=['POST'])
@csrf.exempt
def firebase_auth():
    """Authenticate or register a user using a Firebase ID token (or mock token in dev)."""
    data = request.get_json(silent=True)
    if not data or 'idToken' not in data:
        return invalid_json_error("Missing required parameter: idToken")
        
    id_token = data['idToken']
    is_mock = id_token.startswith('mock-firebase-token-')
    ...
    if is_mock:
        ...
        email = id_token[len('mock-firebase-token-'):]
        if not email or '@' not in email:
            return auth_error("Invalid mock token format. Must be mock-firebase-token-email@domain.com")
        firebase_uid = f"mock-uid-{email}"
        ...
        if not firebase_configured:
            return auth_error("Firebase is not configured on this server, and real ID token verification was requested.")
            
        if not FIREBASE_ADMIN_AVAILABLE:
            return auth_error("firebase-admin package is missing on the server. Real ID token verification cannot be performed.")
        ...
        try:
            ...
            username = decoded_token.get('name') or (email.split('@')[0] if email else f"user_{firebase_uid[:8]}")
            provider = decoded_token.get('firebase', {}).get('sign_in_provider', 'firebase')
        except Exception as e:
            logger.error(f"Failed to verify Firebase ID token: {e}", exc_info=True)
            return auth_error(f"Invalid Firebase ID token: {str(e)}")

    if not email:
        return auth_error("Authentication succeeded, but no email was found in the user profile.")
    ...
    try:
        ...
        if user:
            ...
            db.session.commit()
            logger.info(f"User {user.username} (ID: {user.id}) logged in via Firebase.")
        else:
            # Create a new user
            ...
            db.session.add(user)
            db.session.commit()
            logger.info(f"User {user.username} (ID: {user.id}) registered via Firebase.")
        ...
        db.session.rollback()
        logger.error(f"Database error during Firebase auth: {e}")
        return internal_error("A database error occurred during authentication.")
    except Exception as e:
        db.session.rollback()
        logger.error(f"Unexpected error during Firebase auth: {e}")
        return internal_error(str(e))
